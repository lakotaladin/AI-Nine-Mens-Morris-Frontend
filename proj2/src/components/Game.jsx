...
import "./Game.css";
import placeStoneSound from "../assets/sounds/take.wav";
import removeStoneSound from "../assets/sounds/remove.wav";
...
const placeStoneAudio = new Audio(placeStoneSound);
const removeStoneAudio = new Audio(removeStoneSound);
...
function Stone({ square, index, color, selected, onStoneClick }) {
  ...
  return (
    <circle
      className={selected ? "stone selected-circle" : "stone"}
      cx={x}
      cy={y}
      r={3}
      fill={color}
      strokeWidth={0.5}
      onClick={() => onStoneClick(square, index, color)}
    />
  );
}
...
export default function Game({
  ...
  useEffect(() => {
    ...
    if (pastMovesLength >= 16) {
      const duplicatedFirst = pastMoves.slice(
        pastMovesLength - 16,
        pastMovesLength - 8
      );
      const duplicatedLast = pastMoves.slice(
        pastMovesLength - 8,
        pastMovesLength
      );
      let duplicated = true;
      for (let i = 0; i < 8; i++) {
        if (
          JSON.stringify(duplicatedFirst[i]) !==
          JSON.stringify(duplicatedLast[i])
        ) {
          duplicated = false;
          break;
        }
  ...
  function checkLine(square, index, stoneColor) {
    if (index % 2 !== 0) {
      // centranli
      const prev = stones.find(
        (s) => s.square === square && s.index === index - 1
      );
      const next = stones.find(
        (s) => s.square === square && s.index === (index + 1) % 8
      );
      if (
        prev &&
        next &&
        prev.color === stoneColor &&
        next.color === stoneColor
      ) {
        return true;
      }
  ...
  function onCircleClick(square, index) {
    ...
    if (
      ...
      if (
        ...
        setStones(
          stones.map((stone) =>
            stone.square === selectedStone.square &&
            stone.index === selectedStone.index &&
            stone.color === selectedStone.color
  ...
  function onStoneClick(square, index, stoneColor) {
    ...
    if (removeStoneMode) {
      ...
      setStones(
        stones.filter((s) => !(s.square === square && s.index === index))
      );
    ...
    if (
      (stoneColor === "white" && whiteRemaining > 0) ||
      (stoneColor === "black" && blackRemaining > 0)
    )
      return;
    // ako je vec odabran kamen, a kliknuti kamen je isti kao odabrani, ponisti odabir
    if (
      selectedStone &&
      selectedStone.square === square &&
      selectedStone.index === index &&
      selectedStone.color === stoneColor
    ) {
      setSelectedStone(null);
    } else {
      // inace, postavi novi odabrani kamen
      const newStone = stones.find(
        (s) =>
          s.square === square && s.index === index && s.color === stoneColor
      );
  ...
  function playMove(move) {
    ...
    switch (move[0]) {
      ...
      default:
        console.error("Unknown move type:", move[0]);
    }
  }
      ...
      {showModal && (
        <div ref={modalRef} className="modal">
          <div className="htp-modal">
            <h1 className="title-howtoplay">How to play</h1>
            <p>
              1. PLACE ALL YOUR PIECES <i>MEN</i> ON BOARD AND MOVE THEM TO FORM
              A LINE OF 3 PIECES <i>A MILL</i>.
            </p>
            <br />
            <p>
              {" "}
              2. ONCE YOU FORM A MILL, YOU CAN REMOVE ONE OF YOUR OPPONENT'S
              MEN. BREAK A MILL TO UNLOCK THE MILLS AGAIN.
            </p>
            <br />
            <p>
              3. PIECES FROM A FORMED MILL CAN'T BE REMOVED UNLESS NO OTHER MEN
              ARE AVAILABLE.
            </p>
            <br />
            <p>
              4. YOUR AIM IS TO LEAVE YOUR OPPONENT WITH EITHER 2 PIECES OR 0
              MOVES!
            </p>
            <br />
            <div
        ...
        <div style={{ width: "100%", padding: "0%", margin: "0%" }}>
          ...
          <div className="game-section">
            <div className="player-options">
              ...
              >
                Player 1 <div className="flipping-circle-white"></div>
              </h3>
              <h3>
                {[...Array(whiteRemaining)].map((_, index) => (
                  <div key={index} className="stone-circle-remaining-white">
                    {index + 1}
                  </div>
                ))}
              </h3>
            ...
            <div className="player-options">
              ...
              >
                Player 2 <div className="flipping-circle-black"></div>
              </h3>
              ...
              <h3>
                {[...Array(blackRemaining)].map((_, index) => (
                  <div key={index} className="stone-circle-remaining-black">
                    {index + 1}
                  </div>
                ))}
              </h3>
